...
atom_decoder = ['B','C', 'N', 'O', 'F', 'P', 'S', 'Cl', 'Br',"I"]
...
def validate_data_dimensions(data):
    ...
    logger.info(f"edge_index shape: {data.edge_index.shape}")
    logger.info(f"edge_attr shape: {data.edge_attr.shape}")
    
    if data.x.shape[-1] != len(atom_decoder):
        logger.error(f"loo {len(atom_decoder)}, true {data.x.shape[-1]}")
        return False
    return True
...
class MOSESDataset(InMemoryDataset):
    ...
    def raw_file_names(self):
        return ['chemblv35_train.csv', 'chemblv35_val.csv', 'chemblv35_test.csv']
    ...
    def validate_molecule(self, mol, types, bonds):
        try:
            for atom in mol.GetAtoms():
                symbol = atom.GetSymbol()
                if symbol not in types:
                    logger.warning(f"Unsupported atom type found: {symbol}")
            ...
            for bond in mol.GetBonds():
                if bond.GetBondType() not in bonds:
                    logger.warning(f"Unsupported bond type found: {bond.GetBondType()}")
                    return False, None, None, None
            ...
            return True, type_idx, (row, col), edge_type
            
        except Exception as e:
            logger.error(f"Error validating molecule: {str(e)}")
            return False, None, None, None
    ...
    def process(self):
        ...
        logger.info(f"Processing {self.stage} dataset...")
        logger.info(f"Supported atom types: {self.atom_decoder}")
        ...
        for i, smile in enumerate(tqdm(smiles_list)):
            ...
            if mol is None:
                logger.warning(f"Could not parse SMILES {smile}")
                continue
                
            N = mol.GetNumAtoms()
            is_valid, type_idx, edge_indices, edge_type = self.validate_molecule(mol, types, bonds)
            
            if not is_valid or len(edge_indices[0]) == 0:
                continue
            ...
            if not validate_data_dimensions(data):
                logger.warning(f"skip {i}: fail")
                continue
            ...
            try:
                ...
                logger.error(f"Error processing molecule {i}: {str(e)}")
                continue

        logger.info(f"Successfully processed {len(data_list)} molecules")
        ...
        if self.filter_dataset:
            ...
            logger.info(f"Number of molecules kept: {len(smiles_kept)} / {len(smiles_list)}")
...
class MOSESinfos(AbstractDatasetInfos):
    def __init__(self, datamodule, cfg, recompute_statistics=False, meta=None):
        ...
        self.atom_weights = {0: 10.8, 1: 12, 2: 14, 3: 16, 4: 19, 5: 31, 6: 32.1, 7: 35.45, 8: 79.9, 9:126.9}
        self.valencies = [3,4, 3, 2, 1, 3, 6, 1, 1,1]
        ...
        self.node_types = torch.tensor([0.01, 7.3680e-01, 1.2209e-01, 9.9772e-02, 1.3708e-02, 2.3351e-05, 1.7811e-02,
...
if __name__ == "__main__":
    ds = [MOSESDataset(s, "./dataset/chembl/") for s in ["train", "val", "test"]]