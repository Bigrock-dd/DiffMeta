...
from analysis.rdkit_functions import compute_molecular_metrics_list,gen_smile_list,build_molecule
...
#                f"Marginal distribution of the classes: {x_marginals} for nodes, {e_marginals} for edges")
    ...
    def write_reward(self,rewardavg,rewardstd):
        ...
        if self.cfg.dataset.name in ["zinc","moses","chembl","guacamol"]:
    ...
    def train_step_gdpo(self,data,i):
        ...
        smiles_log = f'main_generate.log'
        
        for loop_idx in range(self.cfg.general.sampleloop):
            X_traj, E_traj, node_mask, rewards, rewardsmean, valid_smiles, rewards_raw, vina_scores ,ml_scores = self.sample_batch_ppo(bs)
            
            s0 = utils.PlaceHolder(X=X_traj[-1], E=E_traj[-1], y=None).mask(node_mask, collapse=True)
            molecule_list = []
            
            with open(smiles_log, 'a') as f:
                f.write(f"\n=== Epoch {self.current_epoch}, Batch {loop_idx} ===\n")
                
                for i in range(bs):
                    n = torch.sum(node_mask[i])
                    atom_types = s0.X[i, :n].cpu()
                    edge_types = s0.E[i, :n, :n].cpu()
                    ...
                    try:
                        if mol is not None:
                            ...
                            if smi is not None:
                                if i < len(valid_smiles) and i < len(vina_scores):
                                    f.write(f"Mol {i}: {smi} (reward: {rewards[i]:.3f}, vina: {vina_scores[i]:.3f}, ml: {ml_scores[i]:.3f})\n")
                                else:
                                    f.write(f"Mol {i}: {smi} (reward: {rewards[i]:.3f}, vina: N/A , ml: N/A)\n")
                                
                                graph_filename = f'graphs/mol_{self.current_epoch}_{loop_idx}_{i}.txt'
                                with open(graph_filename, 'w') as gf:
                                    gf.write(f"SMILES: {smi}\n")
                                    gf.write(f"Reward: {rewards[i]:.3f}\n")
                                    gf.write(f"N={atom_types.shape[0]}\n")
                                    gf.write("X: \n")
                                    gf.write(" ".join(map(str, atom_types.tolist())) + "\n")
                                    gf.write("E: \n")
                                    for bond_list in edge_types.tolist():
                                        gf.write(" ".join(map(str, bond_list)) + "\n")
                    except:
                        f.write(f"Mol {i}: Invalid molecule\n")
                        continue
                
                f.write(f"Batch average reward: {rewardsmean:.3f}\n")
            
            for t, (X, E) in enumerate(zip(X_traj, E_traj)):
                chain_filename = f'chains/chain_{self.current_epoch}_{loop_idx}_step_{t}.txt'
                with open(chain_filename, 'w') as f:
                    f.write(f"Step {t}\n")
                    f.write("X:\n")
                    f.write(str(X.cpu().numpy()))
                    f.write("\nE:\n")
                    f.write(str(E.cpu().numpy()))
        ...
        if self.cfg.general.vallike:
            ...
            if self.cfg.dataset.name in ["zinc", "moses", "chembl","guacamol"]:
                ...
                if self.cfg.dataset.name == "moses":
                    result = mose_evaluate(self.cfg.general.target_prop, valid, None, self.train_fps,verbose=True)
                else:
                    result = prop_evaluate(self.cfg.general.target_prop, valid, None, self.train_fps,verbose=True)
                ...
                elif self.cfg.dataset.name == "chembl":
                    logfile = self.home_prefix + "evaluation_dictchembl.log"
                elif self.cfg.dataset.name == "guacamol":
                    logfile = self.home_prefix + "evaluation_dict_guacamol.log"
                ...
                if test:
                    ...
                    with open(logfile, "a+") as logf:
                        ...
                        write_dict = {
                            ...
                            "hit": result["hit"],
                            "avgml": result["avgml"],
                            "avgdock": result["avgdock"],
                            "avgqed": round(result["avgqed"], 4),
                            "avgsa": round(result["avgsa"], 4),
                            "avglogP": round(result["avglogP"], 4)  
                        }
                        logf.write(json.dumps(write_dict) + "\n")
                else:
                    with open(logfile, "a+") as logf:
                        write_dict = {
                            "lr": self.cfg.train.lr,
                            ...
                            "train_method": self.cfg.general.train_method,
                            "weight_list": "q{}_s{}_n{}_d{}_spa{}".format(*self.cfg.general.weight_list),#qed, sa, novel, ds, spacial
                            "innerloop": self.cfg.general.innerloop,
                            "partial": self.cfg.general.partial,
                            "fix": self.cfg.general.fix,
                            "novelty": result["novelty"],
    ...
    def test_epoch_end_olppo(self, outs) -> None:
        ...
        smile_path = {
            "cyp1a2":r"./dataset/zinc/raw/cyp1a2.txt",
            "cyp2c9":r"./dataset/zinc/raw/cyp2c9.txt",
            "cyp2d6":r"./dataset/zinc/raw/cyp2d6.txt",
            "cyp2c19":r"./dataset/zinc/raw/cyp2c19.txt",
            "cyp3a4":r"./dataset/zinc/raw/cyp3a4.txt",
            "7W4A":r"./dataset/zinc/raw/7W4A.txt"
        }
    ...
    def test_epoch_end_evalproperty(self, outs) -> None:
        ...
        filename = "generated_samples_orig_num{}_seed{}_prop{}.txt".format(
            len(samples), seed, self.cfg.general.target_prop
        )
        with open(filename, 'w') as f:
            for item in samples:
                f.write(f"N={item[0].shape[0]}\n")
        ...
        if self.train_fps is None:
            ...
            train_mols = [Chem.MolFromSmiles(smi) for smi in self.train_smiles]
            self.train_fps = [
                AllChem.GetMorganFingerprintAsBitVect(mol, 2, 1024) for mol in train_mols
            ]

        logfile = "test_property_result.log"

        with open("generated_smiles.csv", "w", newline="", encoding="utf-8") as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(["SMILES"])

        scores_file = "generated_smiles_scores.csv"
        ...
        for idx in range(K):
            ...
            with open("generated_smiles.csv", "a", newline="", encoding="utf-8") as csvfile:
                writer = csv.writer(csvfile)
                for s in valid:
                    writer.writerow([s])
            ...
            df_scores = result.get("molecule_scores", None)
        ...
        print(f"Done. Detailed per-molecule scores saved to {scores_file}.")
    ...
    def sample_batch_ppo(self, batch_size: int):
        ...
        if "ogbg" in self.cfg.dataset.name:
        ...
        elif self.cfg.dataset.name in ["zinc","moses","chembl","guacamol"]:
            if self.train_fps is None:
                assert self.train_smiles is not None
                train_mols = [Chem.MolFromSmiles(smi) for smi in self.train_smiles]
                self.train_fps = [AllChem.GetMorganFingerprintAsBitVect((mol), 2, 1024) for mol in train_mols]
            ...
            if len(valid_list)>0:
                if self.cfg.general.discrete:
                    if self.cfg.dataset.name == "moses":
                        valid_score_list, vina_score_list, ml_score_list = gen_score_disc_listmose(self.cfg.general.target_prop,valid_list,self.cfg.general.thres,self.train_fps,self.cfg.general.weight_list)
                    else:
        ...
        self.model.train()
        return torch.stack(X_traj),torch.stack(E_traj),node_mask,advantages,validmean, valid_list, reward_list, vina_score_list, ml_score_list
